...
@interface ChangeImageViewController ()<UIGestureRecognizerDelegate,UITextFieldDelegate>
{
    CGFloat lastScale;
...
-(void)addView{
    ...
    self.baseImage.frame = CGRectMake(0, 0, screenW, screenH-250);
   ...
   //底部视图
    ...
    self.footView.frame=CGRectMake(0,CGRectGetMaxY(self.baseImage.frame)+50, screenW, 100);
    ...
    for (int i =0; i<6; i++) {
       UIButton* btn=[UIButton buttonWithType:(UIButtonTypeCustom)];
        btn.frame = CGRectMake(i*screenW/6, 0, screenW/6, self.footView.frame.size.height);
...
-(void)btnAction:(UIButton*)sender{
    ...
    if (sender.tag ==100) {
        ...
        FingerDrawLine *touchdrawView = [[FingerDrawLine alloc]initWithFrame:self.baseImage.frame];
        
        touchdrawView.currentPaintBrushColor = [UIColor redColor];
        touchdrawView.currentPaintBrushWidth = 5.0;
        [self.baseImage addSubview:touchdrawView];
...
//-(void)line{
//    self.imageV = [[UIImageView alloc]initWithFrame:CGRectMake(0, 120, 414, 736-150)];
//    self.imageV.image = [UIImage imageNamed:@"22.png"];
//    [self.view addSubview:self.imageV];
//    
//    UIButton *testBtn = [[UIButton alloc]initWithFrame:CGRectMake(414/2.0-60, 60, 120, 36)];
...
-(void)jiantouAction{
    ...
    self.jiantouImage.frame = CGRectMake(0, 0, 150, 150);
...
-(void)yuanquanAaction{
   ...
   self.yuanquanImage.frame = CGRectMake(0, 0, 150, 150);
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    [self.shuiyinTe resignFirstResponder];
    return YES;
