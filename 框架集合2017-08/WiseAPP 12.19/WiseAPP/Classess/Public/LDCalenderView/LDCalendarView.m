...
//        [self addSubview:bgAlphaView];
        ...
        leftImage.image = [UIImage imageNamed:@"home_gerenT_right"];
        leftImage.transform=CGAffineTransformMakeRotation(M_PI);
        [_contentBgView addSubview:leftImage];
        leftImage.frame = CGRectMake(CGRectGetWidth(_contentBgView.frame)/3.0 - 8 , (42-13)/2.0-7, PROPORTION_WIDTH(30), PROPORTION_WIDTH(30));
        
        UIImageView *rightImage = [UIImageView new];
        rightImage.image = [UIImage imageNamed:@"home_gerenT_right"];
        [_contentBgView addSubview:rightImage];
        rightImage.frame = CGRectMake(CGRectGetWidth(_contentBgView.frame)*2/3.0 + 12, (42-13)/2.0-7, PROPORTION_WIDTH(30), PROPORTION_WIDTH(30));
        //_contentBgView.backgroundColor =[UIColor greenColor];
    
        _titleLab = [[UILabel alloc] initWithFrame:CGRectMake(_contentBgView.width/3.0-15, 0, CGRectGetWidth(_contentBgView.frame)/3.0+PROPORTION_WIDTH(60), 42)];
...
//        _dateBgView.backgroundColor=[UIColor yellowColor];
//        _contentBgView.backgroundColor=[UIColor redColor];
        _done = [UIButton buttonWithType:UIButtonTypeCustom];
        [_done setFrame:CGRectMake(0,0,STANDARD_WIDTH,STANDARD_HEIGHT)];
        _done.center = CGPointMake(_contentBgView.width/2, _contentBgView.height-PROPORTION_HIGHT(25) - PROPORTION_HIGHT(50)/2);
        ...
        [_done setTitle:@"选择完成" forState:UIControlStateNormal];
...
//    }
    ...
    for(int i = startDayIndex; i < kTotalNum;i++)
    {
        ...
        {//是1号
            ...
            monthLab.font = [UIFont systemFontOfSize:10];
...
- (void)clickForIndex:(NSInteger)index
{
    ...
    if (index < [_currentMonthDateArray count]) {
        ...
        if ([self.today  compare:date] < 0) {
            //[self.today  compare:date] < 0 取消这个当天前的可以选择
            //时间比今天大,同时是当前月
        }else {
            return;
        }
        if ([_selectArray containsObject:interval]) {
           ...
           [btn setTitleColor:[UIColor hexColorWithString:@"2b2b2b"] forState:UIControlStateNormal];
            ...
            //如果选中的是下个月切换到下个月(在当月点击下月的跳转到下月需要打开下面判断)
//            if (date.month > self.month) {
//                [self rightSwitch];
//            }
        }
    }
    [self btnActionContent];
}
