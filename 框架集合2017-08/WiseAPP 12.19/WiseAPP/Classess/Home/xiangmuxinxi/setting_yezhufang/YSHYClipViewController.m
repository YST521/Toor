...
-(void)viewDidLoad
{
    ...
    [self CreatUI];
    [self addAllGesture];
    self.navigationItem.title = @"图片选择和裁剪";
...
-(void)CreatUI
{
    ...
    [_overView setBackgroundColor:[UIColor clearColor]];
      //[_overView setBackgroundColor:[UIColor redColor]];
    _overView.opaque = NO;
   ...
   // [clipBtn setBackgroundColor:[UIColor redColor]];
    [clipBtn setFrame:CGRectMake(STANDARD_X, self.view.frame.size.height - 50, STANDARD_WIDTH, STANDARD_HEIGHT)];
    [self.view addSubview:clipBtn];
    KUIbutton(clipBtn);
    KUIbuttonClick(clipBtn);
