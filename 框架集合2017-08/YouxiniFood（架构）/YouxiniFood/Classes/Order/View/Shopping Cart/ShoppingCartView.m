...
-(void)layoutUI{
    ...
    self.line = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SceneWidth, 0.5)];
    ...
    [self.shoppingCartBtn setBackgroundImage:[UIImage imageNamed:@"bg_gouwuche"] forState:UIControlStateNormal];
    self.shoppingCartBtn.frame = CGRectMake(20, -10, 50,50);
    ...
    self.OverlayView = [[OverlayView alloc] initWithFrame:CGRectMake(0, 0, SceneWidth, SceneHeight-50)];
...
- (void)payclick:(id)sender {
    if ([self.delegate respondsToSelector:@selector(shoppingClickSelectCompleteWithArray:)]) {
        [self.delegate shoppingClickSelectCompleteWithArray:self.OrderList.objects];
    }
}
...
- (void)shoppCartClick:(id)sender {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        point.y -= (self.OrderList.frame.size.height + 40);
        labelPoint.x -= 60;
...
-(void)updateFrame:(GoodsListView *)orderListView
{
    ...
    float height = 0;
    height = [orderListView.objects count] * 50+50;
    int maxHeight = self.parentView.frame.size.height - 250;
    if (height >= maxHeight) {
        height = maxHeight;
    }
...
-(void)setTotalMoney:(NSInteger)nTotal
{
    ...
    if(self.nTotal > 0)
    {
        ...
        else
        {
            ...
            [self.payButton setTitle:@"选好了" forState:UIControlStateNormal];
            [self.payButton setBackgroundColor:GoldColor];
        }
