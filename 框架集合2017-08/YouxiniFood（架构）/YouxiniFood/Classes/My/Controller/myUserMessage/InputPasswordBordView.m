...
@interface InputPasswordBordView ()

//@property (strong, nonatomic) NSMutableString *textStore;//保存密码的字符串

@end

@implementation InputPasswordBordView
...
- (instancetype)initWithCoder:(NSCoder *)coder {
    ...
    if (self) {
        ...
        //          self.textStore =(NSMutableString*) @"123456";
        self.squareWidth = 45;
        self.passWordNum = 6;
        self.pointRadius = 6;
...
- (void)insertText:(NSString *)text {
    ...
    if (self.textStore.length < self.passWordNum) {
        ...
        if(basicTest) {
            ...
            NSLog(@"************SS--%@",self.textStore);
            if ([self.delegate respondsToSelector:@selector(passWordDidChange:)]) {
                [self.delegate passWordDidChange:self ];
            }
            if (self.textStore.length == self.passWordNum) {
                if ([self.delegate respondsToSelector:@selector(passWordCompleteInput: inPutStr:)]) {
                    [self.delegate passWordCompleteInput:self inPutStr:self.textStore];
                }
            }
            [self setNeedsDisplay];
        }
