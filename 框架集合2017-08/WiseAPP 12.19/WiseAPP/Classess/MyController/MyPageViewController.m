...
-(void)backImageView{
    UIImage *image=[UIImage imageNamed:@"home_back.png"];
   
    
    self.backgroundImgV=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, image.size.height*0.8)];
    self.backgroundImgV.image=image;
    
   self.backgroundImgV.userInteractionEnabled=YES;
    [self.view addSubview:self.backgroundImgV];
   self.backImgHeight=self.backgroundImgV.frame.size.height;
   self.backImgWidth=self.backgroundImgV.frame.size.width;
}
...
-(UIImageView *)headImageView{
    if (!_headImageView) {
        ...
        _headImageView.frame=CGRectMake(0, 64, WIDTH, PROPORTION_HIGHT(170));
        _headImageView.backgroundColor=[UIColor clearColor];
        _headImageView.userInteractionEnabled = YES;
        //_headImageView.image = [UIImage imageNamed:@"my_head_image"];
